...
// src/components/AnalysisViewer/FileList.jsx
import React, { useState, useEffect } from 'react';
import { getFilesAccessed } from '../../api/analysisApi';
import Loader from '../common/Loader';
...
const FileList = ({ sessionId, onFileSelect, selectedFile }) => {
  ...
  const renderFileTree = (node, path = "", depth = 0) => {
    if (typeof node === 'string') {
      ...
      return (
        <div 
          key={node}
          className={`pl-${depth * 4} py-1 cursor-pointer hover:bg-blue-500 hover:text-white ${selectedFile === node ? 'bg-black-100 text-white' : ''}`}
          onClick={() => onFileSelect(node)}
        >
          <span className="text-white-800">📄 {fileName}</span>
        </div>
      );
    }
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      <div className="p-3 border-b bg-gray-50 text-black">
        <input
          type="text"
          placeholder="Search files..."
          value={searchTerm}
      ...
      <div className="overflow-y-auto max-h-[500px] p-2">
        {files.length === 0 ? (
          <div className="text-green-500 text-center py-4">No files found</div>
        ) : (
          renderFileTree(fileTree)
        )}
      </div>
